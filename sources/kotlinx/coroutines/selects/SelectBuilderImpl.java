            ...
            if (obj == SelectKt.UNDECIDED) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_result$FU, this, IntrinsicsKt.getCOROUTINE_SUSPENDED(), SelectKt.RESUMED)) {
                    if (Result.m169isFailureimpl(result)) {
                        Continuation<R> continuation = this.uCont;
                        Throwable thM166exceptionOrNullimpl = Result.m166exceptionOrNullimpl(result);
                        Intrinsics.checkNotNull(thM166exceptionOrNullimpl);
                        Result.Companion companion = Result.INSTANCE;
                        if (DebugKt.getRECOVER_STACK_TRACES() && (continuation instanceof CoroutineStackFrame)) {
                            thM166exceptionOrNullimpl = StackTraceRecoveryKt.recoverFromStackFrame(thM166exceptionOrNullimpl, (CoroutineStackFrame) continuation);
                        }
                        continuation.resumeWith(Result.m163constructorimpl(ResultKt.createFailure(thM166exceptionOrNullimpl)));
                        return;
                    }
