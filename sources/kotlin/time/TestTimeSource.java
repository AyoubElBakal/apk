...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1625plusAssignLRDsOJo(long duration) {
        long j;
        long jM1536toLongimpl = Duration.m1536toLongimpl(duration, getUnit());
        if (jM1536toLongimpl != Long.MIN_VALUE && jM1536toLongimpl != Long.MAX_VALUE) {
            long j2 = this.reading;
            j = j2 + jM1536toLongimpl;
            if ((jM1536toLongimpl ^ j2) >= 0 && (j2 ^ j) < 0) {
                m1624overflowLRDsOJo(duration);
            }
        } else {
            double dM1533toDoubleimpl = this.reading + Duration.m1533toDoubleimpl(duration, getUnit());
            if (dM1533toDoubleimpl > 9.223372036854776E18d || dM1533toDoubleimpl < -9.223372036854776E18d) {
                m1624overflowLRDsOJo(duration);
            }
            j = (long) dM1533toDoubleimpl;
        }
        this.reading = j;
    }
